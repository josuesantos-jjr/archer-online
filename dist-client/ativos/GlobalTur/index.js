...
import { dispararMensagens, saveMessageToFile } from '../../../src/backend/disparo/disparo.js'; // Importando as funções
...
function formatChatId(chatIdInput) {
...
let messageBufferPerChatId = new Map(); // Usa a interface
loadMessageBuffer();
const messageTimeouts = new Map();
// const orçamentoTimeouts = new Map<string, NodeJS.Timeout>(); // Removido
const leadSummaryTimeouts = new Map(); // Mapa para timers de resumo de lead (adicionado contatoId)
...
const AI_SELECTED = infoConfig.AI_SELECTED || `GEMINI`;
...
if (AI_SELECTED === `GEMINI` && !GEMINI_KEY) {
    throw Error(`Você precisa colocar uma key do Gemini no .env! Crie uma gratuitamente em https://aistudio.google.com/app/apikey?hl=pt-br`);
}
if (AI_SELECTED === `GPT` &&
    (!infoConfig.OPENAI_KEY || !infoConfig.OPENAI_ASSISTANT)) {
    throw Error(`Para utilizar o GPT você precisa colocar no .env a sua key da openai e o id do seu assistente.`);
}
// Função para atualizar o arquivo .env
// Função para atualizar infoCliente.json
async function updateInfoCliente(key, value) {
    try {
        const infoPath = path.join(__dirname, 'config', 'infoCliente.json');
...
async function makeRequestWithRetryLead(prompt, chatId, conversation, retries = 0) {
    try {
        ...
        const response = AI_SELECTED === `GPT`
...
async function makeRequestWithRetrySummary(prompt, chatId, conversation, retries = 0) {
    try {
...
async function makeRequestWithRetryName(prompt, chatId, conversation, retries = 0) {
    try {
...
async function makeRequestWithRetryInterest(prompt, chatId, conversation, retries = 0) {
    try {
...
async function makeRequestWithRetryOrçamento(prompt, chatId, conversation, retries = 0) {
    try {
        const intervalo_aleatorio = Math.random() * (20 - 15) + 15;
...
async function getMessages(client, chatId, getMessagesParam = {}) {
...
*/
async function findOrCreateContatoPrincipal(telefone, nome, clientId) {
    ...
    let contato = contatos.find((c) => c.telefone === telefone);
    if (contato) {
...
async function findLeadByChatId(clientId, chatId) {
    ...
    if (fs.existsSync(filePath)) {
        ...
        if (data) {
            try {
                ...
                return leads.find((lead) => lead.chatId === chatId && lead.clientId === clientId);
            }
            catch (error) {
...
async function saveLead(clientId, contatoId, leadData) {
    const now = new Date();
...
async function updateLeadSummary(clientId, leadId, contatoId, summary) {
    const filePath = path.join(__dirname, 'config', 'leads.json');
    if (fs.existsSync(filePath)) {
        const data = fs.readFileSync(filePath, 'utf-8');
        if (data) {
            try {
                const leads = JSON.parse(data);
                const leadIndex = leads.findIndex((lead) => lead.id === leadId && lead.clientId === clientId);
                if (leadIndex !== -1) {
...
async function saveChatMessage(clienteIdCompleto, chatId, messageData, clientePath) {
...
async function handleLeadIdentification(client, clientId, chatId, nome, telefone, listaOrigemId = null, listaOrigemNome = null, tagsIniciais = []) {
...
function startLeadSummaryTimer(client, clientId, chatId, leadId, contatoId) {
    if (!contatoId) {
...
/**
*/
async function generateAndSaveSummary(client, clientId, chatId, leadId, contatoId, clientePath, aiSelected) {
...
function saveMessageBuffer() {
    try {
        const dataToSave = {}; // Adiciona tipo explícito
        messageBufferPerChatId.forEach((value, key) => {
            dataToSave[key] = value;
        });
        fs.writeFileSync(MESSAGE_BUFFER_FILE_PATH, JSON.stringify(dataToSave, null, 2), 'utf-8');
    }
    catch (error) {
        logger.error('Erro ao salvar o buffer de mensagens local:', error);
    }
}
function markMessagesAsAnswered(chatId) {
...
async function sendMessage(client, clientePath, chatId, message) {
    try {
...
export { sendMessage };
const qrCodeTimeouts = new Map();
// Função para limpar os arquivos de QR Code de um cliente específico
async function clearClientQrCodeFiles(clientName) {
            ...
            if (message.type === `chat` &&
                ...
                if (fs.existsSync(ignoredChatIdsFilePath)) {
                    try {
                        ...
                        if (ignoredChatIds.some((id) => formatChatId(message.chatId) === formatChatId(id))) {
                            ...
                            try {
                                ...
                                logger.error('Erro ao ler Dados.json (ignore):', error);
                            }
    ...
    try {
        ...
        while (retryCount < maxRetries) {
            try {
                ...
                if (answerRetry === `_fim` || answerRetry === undefined)
                    return;
...
// Adiciona client como primeiro parâmetro
async function identifyNameAndInterest(client, chatId) {
    if (!clienteIdCompleto) {
        ...
        if (leadPrompt) {
            ...
            if (leadCheckResponse && leadCheckResponse.toUpperCase().includes("LEAD: SIM")) {
                ...
                if (chatData.lead !== 'sim') {
                    ...
                    try {
                        await handleLeadIdentification(client, // Passa o objeto client
                        clienteIdCompleto, chatId, chatData.name, chatData.telefone, null, // listaOrigemId - precisa ser obtido se veio de disparo
...
function updateLeadData(chatId, field, value) {
