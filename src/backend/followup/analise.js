 ...
 */
export async function analisarNecessidadeFollowUp(clienteId) {
...
async function getChatIds(dadosDir) {
    try {
...
export async function removerFollowUp(clientePath, chatId) {
    const filePath = path.join(clientePath, 'config', 'followups.json');
    try {
        if (fsSync.existsSync(filePath)) {
            const data = await fs.readFile(filePath, 'utf-8');
            let followups = JSON.parse(data);
            followups = followups.filter((followup) => followup.chatId !== chatId);
