 ...
 */
export async function gerarMensagemFollowUp(
  clienteId: string,
  chatId: string,
  level: number,
  ...
  if (!clientePath) {
    console.error(
      `[FollowUp Geração] Caminho inválido para cliente ${clienteId}`
    );
    return null;
  }
  ...
  try {
    ...
    if (!config.ativo) return null; // Não gera se inativo
    ...
    if (!promptBase) {
      console.error(
        `[FollowUp Geração] Prompt para nível ${level} não encontrado para cliente ${clienteId}.`
      );
      return null;
    }
    ...
    // Chama a IA
    console.log(
      `[FollowUp Geração] Gerando mensagem nível ${level} para ${chatId} (Cliente: ${clienteId})...`
    );
    ...
    console.log(
      `[FollowUp Geração] Mensagem gerada para ${chatId}: ${mensagemGerada}`
    );
    return mensagemGerada.trim();
  } catch (error) {
    console.error(
      `[FollowUp Geração] Erro ao gerar mensagem de follow-up para ${chatId} (Cliente: ${clienteId}):`,
      error
    );
    return null;
  }
