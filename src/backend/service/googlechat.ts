...
export const mainGoogleChat = async ({
  ...
  while (retries < maxRetries) {
    try {
      ...
      const promptParts: Part[] = [{ text: currentMessageChat }]; // Crie um array de Part
      console.log('Sending prompt parts to Gemini API:', JSON.stringify(promptParts)); // Log do prompt enviado
      ...
      if (
        ...
        text = await mainGoogleChat({
          currentMessageChat: currentMessageChat, // Usar currentMessageChat original
          chatId,
          clearHistory,
          maxRetries,
          __dirname,
      ...
      if (text === lastMessage) {
        ...
        const gereOutraresposta =
          'Gere uma resposta diferente, utilize outras palavras mas de uma resposta diferente que faça sentido com o assunto. é extremamente importante que você não repita a mesma frase. É extremamente importante que você responda como se fosse uma pessoa real';
        const NovaResposta = `${currentMessageChat}. ${gereOutraresposta}`; // Usar currentMessageChat original
      ...
      if (clearHistory) {
        ...
        activeChats.set(chatId, [
          ...activeChats.get(chatId),
          {
            ...
            parts: [
              {
                text: currentMessageChat, // Usar currentMessageChat original
              },
            ],
          },
          {
      ...
      console.error('Erro ao processar a mensagem GoogleChat:', error);
      retries++;
