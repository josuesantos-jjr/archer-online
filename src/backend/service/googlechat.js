...
export const mainGoogleChat = async ({ currentMessageChat, chatId, clearHistory, maxRetries = 500, // Define o número máximo de tentativas
__dirname, }) => {
    let retries = 0;
    while (retries < maxRetries) {
        try {
            const chat = getOrCreateChatSession(chatId, __dirname);
            const promptParts = [{ text: currentMessageChat }]; // Crie um array de Part
            ...
            if (text.includes('{\n') ||
                ...
                text = await mainGoogleChat({
                    currentMessageChat: currentMessageChat,
                    chatId,
                    clearHistory,
                    maxRetries,
                    __dirname,
