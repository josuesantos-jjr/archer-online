...
export const mainGoogleBG = async ({ currentMessageBG, chatId, clearHistory, maxRetries = 500, // Define o número máximo de tentativas
__dirname, }) => {
    let retries = 0;
    while (retries < maxRetries) {
        try {
            const chat = getOrCreateChatSession(chatId, __dirname);
            const promptParts = [{ text: currentMessageBG }]; // Crie um array de Part
