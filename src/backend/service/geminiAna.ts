...
if (!process.env.NEXT_PUBLIC_GEMINI_API_KEY) {
  throw new Error(
    'NEXT_PUBLIC_GEMINI_API_KEY não está definida nas variáveis de ambiente'
  );
}
...
Seja proativa, analítica e forneça respostas claras e úteis. Lembre-se de usar o conteúdo da página atual como contexto adicional sempre que relevante.`;
    ...
    const chatHistory = [
      ...
      { role: 'user', parts: [{ text: systemInstruction }] }, // Start context with system prompt
      {
        role: 'model',
        parts: [
          { text: 'Entendido. Sou Ana, sua assistente. Como posso ajudar?' },
        ],
      }, // Initial model response acknowledging the role
      ...history.map((message) => ({
