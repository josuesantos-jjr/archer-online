...
function splitSimpleMessages(text: string, useSpecialSplit: boolean): string[] {
  ...
  if (useSpecialSplit) {
     const complexPattern =
      /(\([^\)]*\))|(http[s]?:\/\/[^\s]+)|(www\.[^\s]+)|([^\s]+@[^\s]+\.[^\s]+)|(["'].*?["'])|(R\$\s*\d+(?:\.\d{3})*(?:,(?:[0-9]{2})))/g;
...
export async function sendMessagesWithDelay({
  ...
  async function sendMessageWithRetryAndTyping(client: any, messageData: { message: string; delay: number }, __dirname: string, maxRetries = 3): Promise<any> {
