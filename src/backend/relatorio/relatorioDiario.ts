...
async function criarEnviarRelatorioDiario(client: any, clientePath: string, dataRelatorio: Date) {
  try {
    ...
    try {
        ...
        const registros: any[] = JSON.parse(logContent); // Usar tipo DisparoRegistro[] se definido
    ...
    } catch (error: unknown) { // Adiciona tipo unknown
    ...
    if (disparosFalhaWpp > 0) relatorioTexto += `❌ *Falhas (Sem WPP):* ${disparosFalhaWpp}\n`;
    if (disparosFalhaEnvio > 0) relatorioTexto += `⚠️ *Falhas (Erro Envio):* ${disparosFalhaEnvio}\n`;
    ...
    relatorioMensal[dataFormatada] = {
        ...
        disparosFalhaEnvio,
        conversasRespondidas,
        resumoGeralIA: resumoGeralIA || null, // Salva null se não houver resumo
