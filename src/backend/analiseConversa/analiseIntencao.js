 ...
 */
export async function analisarIntencao(conversationHistory, geminiKey, logger // Idealmente, usar um tipo mais específico para o logger
) {
    logger.info('[Analise Intencao] Iniciando análise de intenção...');
    let model = null;
    try {
        if (!geminiKey) {
            logger.warn('[Analise Intencao] Chave Gemini não fornecida. Análise de intenção pode falhar.');
    ...
    if (!model) {
        ...
        logger.error('[Analise Intencao] Modelo Gemini não inicializado após tentativa. Análise cancelada.');
        return 'ERRO_ANALISE_MODELO';
    }
    if (!conversationHistory || conversationHistory.trim().length === 0) {
        logger.warn('[Analise Intencao] Histórico de conversa vazio. Retornando INDETERMINADO.');
        return 'INDETERMINADO';
    }
    try {
        const fullPrompt = promptAnaliseIntencao.replace('{conversation_history}', conversationHistory);
        const chat = model.startChat({ generationConfig, safetySettings });
        const result = await chat.sendMessage(fullPrompt);
        const responseText = result.response.text().trim().toUpperCase();
        logger.info(`[Analise Intencao] Intenção identificada pela IA: ${responseText}`);
        ...
        if (categoriasValidas.includes(responseText)) {
            ...
            logger.warn(`[Analise Intencao] Resposta da IA não corresponde a uma categoria válida: "${responseText}". Retornando OUTRO.`);
            return 'OUTRO';
        }
    }
    catch (error) {
