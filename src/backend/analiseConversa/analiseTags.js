 ...
 */
export async function analisarTags(conversationHistory, geminiKey, logger // Idealmente, usar um tipo mais específico para o logger
) {
    logger.info('[Analise Tags] Iniciando análise de tags...');
    let model = null;
    try {
        if (!geminiKey) {
            logger.warn('[Analise Tags] Chave Gemini não fornecida. Análise de tags pode falhar.');
        }
    ...
    if (!model) {
        ...
        logger.error('[Analise Tags] Modelo Gemini não inicializado após tentativa. Análise cancelada.');
        return [];
    }
    if (!conversationHistory || conversationHistory.trim().length === 0) {
        logger.warn('[Analise Tags] Histórico de conversa vazio. Retornando array vazio.');
        return [];
    }
    try {
        const fullPrompt = promptAnaliseTags.replace('{conversation_history}', conversationHistory);
        ...
        try {
            ...
            if (Array.isArray(tagsArray) &&
                ...
                const formattedTags = tagsArray.map((tag) => tag.toLowerCase().replace(/\s+/g, '-'));
                logger.info(`[Analise Tags] Tags formatadas: ${formattedTags.join(', ')}`);
                return formattedTags;
            }
            else {
                logger.warn(`[Analise Tags] Resposta da IA não é um array de strings válido: "${responseText}". Retornando array vazio.`);
                return [];
            }
        }
        catch (parseError) {
            logger.error(`[Analise Tags] Erro ao parsear JSON de tags da IA: ${parseError}. Resposta recebida: ${responseText}`);
            return [];
        }
    }
    catch (error) {
