...
export async function monitorarConversa(clientePath, chatId, listaNome, client) {
    try {
        ...
        if (agendamento.data_agendada && agendamento.horario_agendado) {
            ...
            const agendamentoExistente = agendamentos.agendamentos.find((a) => a.chatId === chatId);
        ...
        const monitoramentoPath = path.join(clientePath, 'Chats', 'Historico', chatId, 'monitoramento.json');
        let historico = [];
        try {
