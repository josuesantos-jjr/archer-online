...
const extrairValorNumerico = (texto) => {
    const numeros = texto.match(/\d+/g);
    return numeros ? parseInt(numeros[0]) : 0;
};
// Função para salvar mensagens em um arquivo (mantida como estava)
export function saveMessageToFile(client, clientePath, chatId, message, type, contato, listaNome) {
...
const gerarTempoAleatorio = (intervaloDe, intervaloAte) => {
...
const verificarContaWhatsApp = async (client, telefone) => {
    try {
...
const dentroDoHorario = (horarioInicial, horarioFinal) => {
...
const diaDaSemanaValido = (diaInicial, diaFinal, dataParaVerificar) => {
...
const salvarLog = (mensagem, clientePath) => {
    try {
...
const registrarEstado = (estado, clientePath) => {
...
const salvarEstado = (estado, clientePath) => {
    try {
...
const carregarEstado = (clientePath) => {
...
export function getPasta(cliente) {
    let nomeFinalCliente;
    if (cliente.includes('/')) {
...
const dispararMensagens = async (client, clientePath) => {
    ...
    try {
        ...
        const horarioInicial = typeof regras.HORARIO_INICIAL === 'string' ? regras.HORARIO_INICIAL : REGRAS_DISPARO_PADRAO.HORARIO_INICIAL;
        const horarioFinal = typeof regras.HORARIO_FINAL === 'string' ? regras.HORARIO_FINAL : REGRAS_DISPARO_PADRAO.HORARIO_FINAL;
        const diaInicial = typeof regras.DIA_INICIAL === 'string' ? regras.DIA_INICIAL.toLowerCase() : REGRAS_DISPARO_PADRAO.DIA_INICIAL;
        const diaFinal = typeof regras.DIA_FINAL === 'string' ? regras.DIA_FINAL.toLowerCase() : REGRAS_DISPARO_PADRAO.DIA_FINAL;
        const intervaloDe = extrairValorNumerico(String(regras.INTERVALO_DE ?? REGRAS_DISPARO_PADRAO.INTERVALO_DE));
        const intervaloAte = extrairValorNumerico(String(regras.INTERVALO_ATE ?? REGRAS_DISPARO_PADRAO.INTERVALO_ATE));
        const quantidadeInicial = extrairValorNumerico(String(regras.QUANTIDADE_INICIAL ?? REGRAS_DISPARO_PADRAO.QUANTIDADE_INICIAL));
        const diasAquecimento = extrairValorNumerico(String(regras.DIAS_AQUECIMENTO ?? REGRAS_DISPARO_PADRAO.DIAS_AQUECIMENTO));
        const quantidadeLimite = extrairValorNumerico(String(regras.QUANTIDADE_LIMITE ?? REGRAS_DISPARO_PADRAO.QUANTIDADE_LIMITE));
        const quantidadeSequencia = extrairValorNumerico(String(regras.QUANTIDADE_SEQUENCIA ?? REGRAS_DISPARO_PADRAO.QUANTIDADE_SEQUENCIA));
        const disparoEstrategia = typeof regras.DISPARO_ESTRATEGIA === 'string' ? regras.DISPARO_ESTRATEGIA : REGRAS_DISPARO_PADRAO.DISPARO_ESTRATEGIA;
        const disparoListasSelecionadasRaw = typeof regras.DISPARO_LISTAS_SELECIONADAS === 'string' ? regras.DISPARO_LISTAS_SELECIONADAS : REGRAS_DISPARO_PADRAO.DISPARO_LISTAS_SELECIONADAS;
        const disparoListasSelecionadas = disparoListasSelecionadasRaw ? disparoListasSelecionadasRaw.split(',').map(s => s.trim()).filter(Boolean) : [];
        ...
        while (true) {
            ...
            if (!dataUltimoDisparo || !isSameDay(dataAtual, dataUltimoDisparo)) {
                ...
                if (estado.diasRestantesAquecimento > 0)
                    estado.diasRestantesAquecimento--;
                estado.diaspassados++;
            ...
            for (const nomeArquivoLista of todosArquivosJsonRecount) {
                ...
                try {
                    ...
                    if (Array.isArray(lista.contatos)) {
                        disparosContadosHoje += lista.contatos.filter((c) => c.disparo === "sim" && c.data_1_contato?.startsWith(hojeDataStringRecount)).length; // Verifica início da string de data/hora
                    }
                }
            ...
            if (disparosContadosHoje >= quantidadeMensagensDia) { // Compara com recontagem
                ...
                while (!diaDaSemanaValido(diaInicial, diaFinal, dataVerificar)) {
            ...
            if (indiceListaAtual < listaArquivosFiltrados.length) {
                ...
                if (!Array.isArray(lista.contatos) || lista.contatos.length === 0)
                    continue; // Pula lista vazia
                ...
                for (let i = indiceInicio; i < lista.contatos.length; i++) {
                    ...
                    try {
                        ...
                        registrarDisparo(clientePath, {
                            ...
                            status: sucessoDisparo,
                            etapaAquecimento: diasAquecimento - estado.diasRestantesAquecimento,
                            quantidadeDisparada: estado.contadorMensagens,
                        ...
                        if (targetChatId) {
                            try {
                                ...
                                const disparadosLista = (listaAtualizada.contatos || []).filter((c) => c.disparo === "sim").length;
...
const iniciarDisparoComRecuperacao = async (client, clientePath) => {
    while (true) {
        try {
