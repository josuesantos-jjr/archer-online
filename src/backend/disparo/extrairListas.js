...
async function extrairDadosDaTabela(url) {
...
export async function extrairListas(arquivoListas, clientePath) {
    const listasProcessadas = [];
    try {
        ...
        for (const lista of listas) {
            try {
                ...
                const caminhoJSON = path.join(clientePath, 'config', 'listas', 'download', `${nomeLista}.json`);
                // Criar o diretório "listas/disparar" se ele não existir
                const diretorioListas = path.join(clientePath, 'config', 'listas', 'download');
                if (!fs.existsSync(diretorioListas)) {
                    fs.mkdirSync(diretorioListas, { recursive: true });
                }
                // Salvar o arquivo JSON
                await fs.promises.writeFile(caminhoJSON, JSON.stringify(dadosTabela, null, 2));
...
export async function executarExtracao(arquivoListas, clientePath) {
